...
const ExamPage = () => {
    ...
    useEffect(() => {
        if (location.state) {
            const { classCode, phoneNumber, userName, examId, duration, questions } = location.state;
...
const handleConfirmSubmit = async () => {
    ...
    if (isSubmitted) {
        console.log("Bài thi đã được submit rồi!");
        return;
    }
    ...
    console.log("Bắt đầu gửi API khi nhấn Submit...");
    const response = await submitExamData(examDataSubmit);
    console.log("API response khi nhấn Submit:", response);

    setResultMessage(response.message);
    setIsResultModalOpen(true);
    console.log("Dữ liệu gửi đi khi nhấn Submit:", JSON.stringify(examDataSubmit, null, 2));
};

// Hàm xử lý khi hết thời gian làm bài
const handleTimeExpired = async () => {
    console.log("Hết thời gian làm bài! Tự động submit...");
    
    // Kiểm tra xem đã auto submit chưa (sử dụng ref để tránh gọi nhiều lần)
    if (hasAutoSubmitted.current || isSubmitted) {
        console.log("Bài thi đã được submit rồi, không submit lại!");
        return;
    }
    ...
    try {
        ...
        console.log("Bắt đầu gửi API khi hết thời gian...");
        const response = await submitExamData(examDataSubmit);
        console.log("API response khi hết thời gian:", response);

        // Hiển thị thông báo hết thời gian
        setResultMessage("Hết thời gian làm bài! Bài thi đã được tự động nộp.");
        setIsResultModalOpen(true);
        console.log("Dữ liệu gửi đi khi hết thời gian:", JSON.stringify(examDataSubmit, null, 2));
    } catch (error) {
        console.error("Lỗi khi tự động submit:", error);
        setResultMessage("Đã xảy ra lỗi khi tự động nộp bài. Vui lòng liên hệ giám thị.");
        setIsResultModalOpen(true);
    }
};
